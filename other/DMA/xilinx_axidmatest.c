			     ...
			     unsigned int counter, bool is_srcbuf)
{
	...
	if (is_srcbuf)
		printk(KERN_ERR "%s: srcbuf[0x%x] overwritten! Expected %02x, got %02x\n",
			thread_name, index, expected, actual);
	else if ((pattern & PATTERN_COPY) &&
		 (diff & (PATTERN_COPY | PATTERN_OVERWRITE)))
		printk(KERN_ERR "%s: dstbuf[0x%x] not copied! Expected %02x, got %02x\n",
			thread_name, index, expected, actual);
	else if (diff & PATTERN_SRC)
		printk(KERN_ERR "%s: dstbuf[0x%x] was copied! Expected %02x, got %02x\n",
			thread_name, index, expected, actual);
	else
		printk(KERN_ERR "%s: dstbuf[0x%x] mismatch! Expected %02x, got %02x\n",
			thread_name, index, expected, actual);
}
				   ...
				   u8 pattern, bool is_srcbuf)
{
	...
	if (error_count > 32)
		printk(KERN_ERR "%s: %u errors suppressed\n",
...
static int dmatest_slave_func(void *data)
{
	...
	while (!kthread_should_stop() &&
		...
		if (1 << align > test_buf_size) {
			printk(KERN_ERR "%u-byte buffer too small for %d-byte alignment\n",
		...
		if (!rxd || !txd) {
			...
			for (i = 0; i < dst_cnt; i++)
				dma_unmap_single(rx_dev->dev, dma_dsts[i],
						 test_buf_size,
						 DMA_BIDIRECTIONAL);
			printk(KERN_ERR "%s: #%u: prep error with src_off=0x%x ",
				thread_name, total_tests - 1, src_off);
			printk(KERN_ERR "dst_off=0x%x len=0x%x\n",
				dst_off, len);
			msleep(100);
			failed_tests++;
			continue;
		...
		if (dma_submit_error(rx_cookie) ||
			...
			printk(KERN_ERR "%s: #%u: submit error %d/%d with src_off=0x%x ",
		...
		if (tx_tmo == 0) {
			printk(KERN_ERR "%s: #%u: tx test timed out\n",
				thread_name, total_tests - 1);
			failed_tests++;
			continue;
		} else if (status != DMA_COMPLETE) {
			printk(KERN_ERR "%s: #%u: tx got completion callback, ",
				thread_name, total_tests - 1);
			printk(KERN_ERR "but status is \'%s\'\n",
		...
		if (rx_tmo == 0) {
			printk(KERN_ERR "%s: #%u: rx test timed out\n",
				thread_name, total_tests - 1);
			failed_tests++;
			continue;
		} else if (status != DMA_COMPLETE) {
			printk(KERN_ERR "%s: #%u: rx got completion callback, ",
				thread_name, total_tests - 1);
			printk(KERN_ERR "but status is \'%s\'\n",
				status == DMA_ERROR ? "error" :
				"in progress");
		...
		error_count = 0;
		start = ktime_get();
		printk(KERN_ERR "%s: verifying source buffer...\n", thread_name);
		...
		printk(KERN_ERR "%s: verifying dest buffer...\n",
			 thread->task->comm);
		...
		if (error_count) {
			printk(KERN_ERR "%s: #%u: %u errors with ",
				thread_name, total_tests - 1, error_count);
			printk(KERN_ERR "src_off=0x%x dst_off=0x%x len=0x%x\n",
				src_off, dst_off, len);
			failed_tests++;
		} else {
			printk(KERN_ERR "%s: #%u: No errors with ",
				 thread_name, total_tests - 1);
			printk(KERN_ERR "src_off=0x%x dst_off=0x%x len=0x%x\n",
				 src_off, dst_off, len);
		}
...
static void dmatest_cleanup_channel(struct dmatest_chan *dtc)
{
	...
	list_for_each_entry_safe(thread, _thread, &dtc->threads, node) {
		...
		printk(KERN_ERR "dmatest: thread %s exited with status %d\n",
				     ...
				     struct dmatest_chan *rx_dtc)
{
	...
	if (!thread) {
		printk(KERN_ERR "dmatest: No memory for slave thread %s-%s\n",
	...
	if (IS_ERR(thread->task)) {
		printk(KERN_ERR "dmatest: Failed to run thread %s-%s\n",
			dma_chan_name(tx_chan), dma_chan_name(rx_chan));
		kfree(thread);
		return ret;
	}
				      ...
				      struct dma_chan *rx_chan)
{
	...
	if (!tx_dtc) {
		printk(KERN_ERR "dmatest: No memory for tx %s\n",
			dma_chan_name(tx_chan));
		return -ENOMEM;
	}
	...
	if (!rx_dtc) {
		printk(KERN_ERR "dmatest: No memory for rx %s\n",
			dma_chan_name(rx_chan));
		return -ENOMEM;
	}
	...
	printk(KERN_ERR "dmatest: Started %u threads using %s %s\n",
...
static int xilinx_axidmatest_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(chan)) {
		err = PTR_ERR(chan);
        printk(KERN_ERR "xilinx_dmatest: request TX channel error, err=%d\n",err);
		if (err != -EPROBE_DEFER)
			printk(KERN_ERR "xilinx_dmatest: No Tx channel\n");
		return err;
	}
	...
	if (IS_ERR(rx_chan)) {
		err = PTR_ERR(rx_chan);
        printk(KERN_ERR "xilinx_dmatest: request RX channel error, err=%d\n",err);
		if (err != -EPROBE_DEFER)
			printk(KERN_ERR "xilinx_dmatest: No Rx channel\n");
		goto free_tx;
	}

	err = dmatest_add_slave_channels(chan, rx_chan);
	if (err) {
		printk(KERN_ERR "xilinx_dmatest: Unable to add channels\n");
		goto free_rx;
	}
...
static int xilinx_axidmatest_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry_safe(dtc, _dtc, &dmatest_channels, node) {
		...
		printk(KERN_ERR "xilinx_dmatest: dropped channel %s\n",
			dma_chan_name(chan));
