...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	// printk(KERN_ERR "(irq=%d, dev 0x%p) <<<< ISR.\n", irq, dev_id);


	if (user_irq&&ch_irq==0) {
		if(!isFinish){
			return IRQ_HANDLED;
		}
...
static int irq_msi_setup(struct xdma_dev *xdev, struct pci_dev *pdev)
{
	...
	rv = request_irq(pdev->irq, xdma_isr, IRQF_TRIGGER_FALLING, xdev->mod_name, xdev);
...
static int irq_legacy_setup(struct xdma_dev *xdev, struct pci_dev *pdev)
{
	...
	printk(KERN_ERR "legacy setup request_irq %d\n",rv);
			 ...
			 struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	// printk(KERN_ERR "xdev->h2c_channel_max:%d,,xdev->c2h_channel_max:%d\n",xdev->h2c_channel_max,xdev->c2h_channel_max);
	if (write == 1) {
	...
	// printk(KERN_ERR "debug: channel:%d, h2c_channel_max:%d,c2h_channel_max:%d\n",channel,xdev->h2c_channel_max,xdev->c2h_channel_max);
	if (!engine) {
		pr_err("dma engine NULL\n");
		return -EINVAL;
	}
...
ssize_t my_xdma_xfer_submit(void *dev_hndl, int channel, u64 ep_addr,
    ...
    if (!engine || engine->magic != MAGIC_ENGINE || engine->dir != DMA_TO_DEVICE) {
        pr_err("DMA engine invalid or direction mismatch.\n");
        return -EINVAL;
    }
    ...
    rv = transfer_init(engine, req, &req->tfer[0]);
    if (rv < 0) {
		pr_err("error: transfer init error.\n");
        mutex_unlock(&engine->desc_lock);
        xdma_request_free(req);
        return rv;
    }

    rv = transfer_queue(engine, &req->tfer[0]);
    if (rv < 0) {
		pr_err("error: transfer queue error.\n");
        mutex_unlock(&engine->desc_lock);
        xdma_request_free(req);
        return rv;
    }

	// pr_err("debug:  transfer queue success.\n");
    if (engine->cmplthp){
		// pr_err("debug:  engine->cmplthp.\n");
        xdma_kthread_wakeup(engine->cmplthp);
	}

    if (timeout_ms > 0){
		// pr_err("debug:  timeout>0.\n");
        xlx_wait_event_interruptible_timeout(req->tfer[0].wq,
                                             (req->tfer[0].state != TRANSFER_STATE_SUBMITTED),
                                             msecs_to_jiffies(timeout_ms));
	}
    else{
		// pr_err("debug:  timeout<=0.\n");
        xlx_wait_event_interruptible(req->tfer[0].wq,
                                     (req->tfer[0].state != TRANSFER_STATE_SUBMITTED));
	}

    // 处理传输完成状态
	// pr_err("debug: req->tfer[0].state:%d .\n",req->tfer[0].state);
    switch (req->tfer[0].state) {
    case TRANSFER_STATE_COMPLETED:
        done = req->tfer[0].len;
        rv = 0;
        break;
    ...
    engine->desc_used -= req->tfer[0].desc_num;
	// pr_err("debug: ready destory.\n");
    transfer_destroy(xdev, &req->tfer[0]);
...
EXPORT_SYMBOL(my_xdma_xfer_submit);
