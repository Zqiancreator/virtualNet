		...
		size_t count, loff_t *pos, bool write)
{
  ...
  if(rv){
	...
	unsigned long buff = (unsigned long)buf;
    pages_nr = ((buff + count + PAGE_SIZE - 1) - (buff & PAGE_MASK)) >> PAGE_SHIFT;
    if (pages_nr == 0)
        return -EINVAL;
    ...
    sg = cb.sgt.sgl;
	unsigned int i = 0;
    for (; i < pages_nr; i++, sg = sg_next(sg)) {
        unsigned int offset = offset_in_page(buff);
        unsigned int nbytes = min_t(unsigned int, PAGE_SIZE - offset, count);

        sg_set_page(sg, virt_to_page((void *)buff), nbytes, offset);
	...
	if (rv < 0)
		return rv;
	//end debug
	// printk(KERN_ERR "debug engine->channel:%d\n",engine->channel);// 0
	res = xdma_xfer_submit(xdev, engine->channel, write, *pos, &cb.sgt,
				0, write ? h2c_timeout * 1000 :
					   c2h_timeout * 1000);
